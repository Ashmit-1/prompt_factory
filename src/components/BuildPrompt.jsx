  ...
  const handleSave = async (mode, customVersionName = "") => {
    ...
    if (mode === "overwrite" || existingIdx === -1) {
      ...
      const version =
        existingIdx === -1
          ? "v1"
          : existingPrompts[existingIdx].version || "v1";
      ...
      const nextVersionNumber = versionMatch
        ? parseInt(versionMatch[1]) + 1
        : 2;
    ...
    <div className="build-prompt-container">
      ...
      <div className="build-prompt-grid">
          ...
          <div className="tab-content">
            {activeTab === "prompt" ? (
              <div className="block-editor">
                ...
                <button className="add-block-btn" onClick={addCustomBlock}>
                  + Add Custom Block
                </button>
              </div>
            ) : (
              <div className="variables-editor">
                {Object.keys(variables).length === 0 ? (
                  <div className="empty-state">
                    {
                      "No variables detected. Use {{variable_name}} in your prompt."
                    }
                  </div>
                ) : (
                    ...
                    {Object.entries(variables).map(([key, value]) => (
                      <div key={key} className="variable-row">
                        ...
                        <input
                          ...
                          value={value}
                          onChange={(e) =>
                            updateVariableValue(key, e.target.value)
                          }
                          placeholder="Value..."
                        />
                      </div>
        ...
        <div className="preview-column">
          ...
          <pre className="markdown-preview">{compiledMarkdown()}</pre>
        </div>
      </div>
        ...
        <div className="modal-overlay">
          <div className="save-modal">
            ...
            <p>
              ...
              <strong>{promptName || "this prompt"}</strong>?
            </p>
            <div className="modal-options">
              <button onClick={() => handleSave("overwrite")}>
                Modify Existing / Save New
              </button>
